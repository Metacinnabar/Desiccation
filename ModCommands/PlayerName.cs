...
{
	public class PlayerName : ModCommand
	{
		public override CommandType Type => CommandType.Chat;
		public override string Command => "playername";
		public override string Usage => "/playername NEW NAME (under 20 characters)";
		public override string Description => "Changes your ingame player name";
		...
		public override void Action(CommandCaller caller, string input, string[] args)
		{
			if (ModContent.GetInstance<DesiccationGlobalConfig>().PlayerNameChange)
			{
				if (args.Length > 0)
				{
					...
					if (name.Length <= 20)
					{
						DUtils.Chat($"Changed {caller.Player.name}'s name to '{name}'!");
